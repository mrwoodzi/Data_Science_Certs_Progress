...
class Category:
  ...
  def deposit(self, amount=float, description=None): # instance attribute, also called class method?
    ...
    to_balance = amount
    self.str = to_ledger
    return self.ledger.append(to_ledger), self.to_balance(to_balance), self.str
  ...
  def ledger(self, to_ledger):
    return self.ledger
    #new_ledger = [item [1:-1] for item in self.ledger]
    #return self.ledger
  def str(self, to_ledger):
  ...
  def __str__(self, to_ledger):
    ...
    space = """ """
    #print(line)
    self.ledger = self.str
    return line, self.ledger
