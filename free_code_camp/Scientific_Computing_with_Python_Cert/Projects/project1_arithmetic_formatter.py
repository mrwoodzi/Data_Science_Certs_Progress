#Below code was rewritten on 2/12/2024 with the help of AI. 


def arithmetic_arranger(problems, showResult = False):
  if len(problems) > 5:
      return "Error: Too many problems."
  #split iter into arthimetic operator and numbers
  for iter in problems:
      if iter.split()[1] not in ["+", "-"]:
          return "Error: Operator must be '+' or '-'."
      if not iter.split()[0].isdigit() or not iter.split()[2].isdigit():
          return "Error: Numbers must only contain digits."
      if len(iter.split()[0]) > 4 or len(iter.split()[2]) > 4:
  ...
  fourthLine = ""
  for iter in problems:
      firstNumber = iter.split()[0]
      operator = iter.split()[1]
      secondNumber = iter.split()[2]
      if operator == "+":
      ...
      secondLine += operator + " " + str(secondNumber.rjust(length - 2)) + "    "
      thirdLine += "-" * length + "    "
      fourthLine += str(result.rjust(length)) + "    "
  if showResult:
  ...
  #########Below was my first attempt at coding the function . . . It worked but was a total disaster. I will keep the below code for reference as to my progress
    # op_plus = "+"
    # op_minus = "-"
    # mylist_0 = []
    # mystring_0 = ""
    ...
    #     llenf += len_fstring
    # lleng_int = [
    #     eval(i) for i in lleng
    # ]  # gives me the len of the top part of the problem
    ...
    # for a, b, c, d in zip(
    #     lleng_int, llenf_int, lenlist_lines, sum_len_list
    # ):  # compiles all whitespace
    #     if c == "3":
    #         whitespace_c = "  "
    #         whitespace_f = " "
    ...
    # )
    # problems_list1 = (
    #     new_mylist0 + carriage_return + new_mylist12 + carriage_return + new_mylines
    # )
